# 混合模式实现 - 综合测试计划

> **创建日期**: 2026-02-05
> **版本**: 1.0.0
> **状态**: ✅ 已完成
> **最后更新**: 2026-02-05
> **负责人**: 开发团队

---

## 📋 文档概述

本文档定义了 Claude CLI Provider 混合模式实现的综合测试计划，包括单元测试、集成测试、性能测试和验收测试。

---

## 🎯 测试目标

### 主要目标

1. ✅ **功能完整性** - 验证所有功能按设计工作
2. ✅ **质量保证** - 确保代码质量和稳定性
3. ✅ **性能验证** - 确认性能指标达标
4. ✅ **安全合规** - 验证安全控制有效
5. ✅ **生产就绪** - 确保系统可以安全部署

### 测试范围

**包含**:
- ✅ OpenAI 兼容模式的所有功能
- ✅ Agent 模式的所有功能
- ✅ 核心组件的逻辑正确性
- ✅ API 端点的功能验证
- ✅ 性能指标达标
- ✅ 错误处理和边界情况
- ✅ 安全控制有效性

**不包含**:
- ❌ Claude CLI 本身的测试
- ❌ 第三方库的测试
- ❌ 压力测试（超出性能测试范围）
- ❌ 安全渗透测试

---

## 📊 当前测试状态

### 已完成的测试

| 测试类型 | 数量 | 通过率 | 状态 |
|---------|------|--------|------|
| **单元测试** | 170 | 100% | ✅ 完成 |
| **集成测试场景** | 17 | - | 📝 已创建 |
| **性能测试套件** | 3 | - | 📝 已创建 |

### 测试覆盖率

| 模块 | 语句覆盖 | 分支覆盖 | 状态 |
|------|---------|---------|------|
| 核心逻辑 | 95%+ | 88%+ | ✅ 优秀 |
| 适配器层 | 0% | 0% | ⚠️ 待集成测试 |
| 路由层 | 0% | 0% | ⚠️ 待集成测试 |
| **总体** | **45.12%** | **49.17%** | ⚠️ 需集成测试 |

---

## 🧪 测试策略

### 测试金字塔

```
        /\
       /  \
      / E2E \        3 个性能测试
     /  5%   \
    /----------\
   /  集成测试   \    17 个场景
  /     25%      \
 /----------------\
/   单元测试        \  170 个测试
/      70%          \
/--------------------\
```

**比例分配**:
- **单元测试 (70%)**: 测试独立组件和函数
- **集成测试 (25%)**: 测试组件间交互
- **性能测试 (5%)**: 验证性能指标

### 测试类型

#### 1. 单元测试 ✅

**目标**: 验证每个函数和类的独立行为

**已覆盖**:
- ✅ MessageFormatter - 28 个测试
- ✅ ResponseTransformer - 38 个测试
- ✅ SessionManager - 40 个测试
- ✅ ScreenParser - 65 个测试
- ✅ Logger - 12 个测试
- ✅ Errors - 14 个测试
- ✅ ProcessManager - 10 个测试

**通过率**: 100% (170/170)

#### 2. 集成测试 📝

**目标**: 验证组件间的交互和端到端功能

**已创建**:
- 📝 OpenAI 模式集成测试 - 8 个场景
- 📝 Agent 模式集成测试 - 9 个场景

**状态**: 代码已编写，待执行

#### 3. 性能测试 📝

**目标**: 验证系统性能指标

**已创建**:
- 📝 响应时间测试 - 5 个指标
- 📝 并发测试 - 3 个场景
- 📝 内存测试 - 3 个场景

**状态**: 代码已编写，待执行

---

## 🔬 详细测试计划

### 阶段 1: 环境准备 (预计 30 分钟)

#### 1.1 测试环境搭建

**前置条件**:
- [ ] Node.js v18+ 已安装
- [ ] Claude CLI 已安装并可用
- [ ] API Key 已配置（可选）
- [ ] 端口 3912 可用

**检查清单**:
```bash
# 1. 检查 Node.js 版本
node --version  # 应该 >= v18

# 2. 检查 Claude CLI
claude --version

# 3. 检查端口占用
lsof -i :3912  # 应该为空或可终止

# 4. 安装依赖
npm install

# 5. 运行单元测试验证
npm test
```

#### 1.2 配置验证

**环境变量**:
```bash
# 可选配置
export API_KEY="test-key"           # API 认证
export LOG_LEVEL="info"             # 日志级别
export NODE_ENV="development"       # 环境
export CLAUDE_BIN="claude"          # Claude CLI 路径
export MAX_PROCESSES="5"            # 进程池大小
export MAX_SESSIONS="10"            # 最大会话数
```

**验证步骤**:
- [ ] 环境变量已设置
- [ ] 配置文件正确加载
- [ ] 日志输出正常

---

### 阶段 2: 单元测试验证 (预计 15 分钟)

#### 2.1 运行单元测试

**命令**:
```bash
npm test
```

**预期结果**:
```
Test Suites: 7 passed, 7 total
Tests:       170 passed, 170 total
Time:        < 5s
```

**验证标准**:
- [ ] 所有测试通过（170/170）
- [ ] 无错误或警告
- [ ] 执行时间 < 5 秒

#### 2.2 覆盖率检查

**命令**:
```bash
npm test -- --coverage --coverageReporters=text
```

**预期覆盖率**:
- [ ] 核心组件 > 95%
- [ ] MessageFormatter = 100%
- [ ] SessionManager > 95%
- [ ] ScreenParser > 90%

---

### 阶段 3: 集成测试执行 (预计 1-2 小时)

#### 3.1 启动测试服务器

**终端 1**:
```bash
# 启动服务器
npm start

# 预期输出:
# Server running on http://0.0.0.0:3912
# Environment: development
```

**验证**:
- [ ] 服务器成功启动
- [ ] 监听端口 3912
- [ ] 无错误日志

#### 3.2 OpenAI 模式集成测试

**测试文件**: `tests/integration/openai-mode.test.js`

**场景清单**:

##### 场景 1: 健康检查
```bash
curl http://localhost:3912/health
```
**预期**:
```json
{
  "status": "ok",
  "timestamp": "2026-02-05T...",
  "uptime": ...,
  "version": "2.0.0"
}
```
**验证**:
- [ ] 状态码 200
- [ ] 响应格式正确
- [ ] uptime > 0

---

##### 场景 2: 基础聊天请求（非流式）
```bash
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "sonnet",
    "messages": [
      {"role": "user", "content": "Say \\"Hello World\\""}
    ],
    "stream": false
  }'
```
**预期**:
```json
{
  "id": "chatcmpl-...",
  "object": "chat.completion",
  "created": ...,
  "model": "claude-sonnet",
  "choices": [{
    "index": 0,
    "message": {
      "role": "assistant",
      "content": "Hello World"
    },
    "finish_reason": "stop"
  }],
  "usage": {...}
}
```
**验证**:
- [ ] 状态码 200
- [ ] 响应格式符合 OpenAI 规范
- [ ] content 包含 "Hello"
- [ ] finish_reason = "stop"

---

##### 场景 3: 流式聊天请求
```bash
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "sonnet",
    "messages": [
      {"role": "user", "content": "Count to 5"}
    ],
    "stream": true
  }'
```
**预期**:
```
data: {"id":"chatcmpl-...","choices":[...],"object":"chat.completion.chunk"}

data: {"id":"chatcmpl-...","choices":[...],"object":"chat.completion.chunk"}

...

data: [DONE]
```
**验证**:
- [ ] 状态码 200
- [ ] Content-Type: text/event-stream
- [ ] 多个 data 块
- [ ] 最后一个是 `[DONE]`
- [ ] 内容连贯

---

##### 场景 4: 系统提示词
```bash
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "sonnet",
    "messages": [
      {"role": "system", "content": "You are a pirate. Always speak like a pirate."},
      {"role": "user", "content": "Hello"}
    ],
    "stream": false
  }'
```
**验证**:
- [ ] 响应包含海盗用语（如 "Ahoy", "Matey"）
- [ ] 系统提示词生效

---

##### 场景 5: 多轮对话
```bash
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "sonnet",
    "messages": [
      {"role": "user", "content": "My name is Alice"},
      {"role": "assistant", "content": "Hello Alice!"},
      {"role": "user", "content": "What is my name?"}
    ],
    "stream": false
  }'
```
**验证**:
- [ ] 响应包含 "Alice"
- [ ] 上下文正确传递

---

##### 场景 6: API Key 认证
```bash
# 设置 API Key
export API_KEY="secret-key"

# 启动服务器
npm start

# 无 Key 请求
curl http://localhost:3912/health
# 预期: 200

# 需要 Key 的请求
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"sonnet","messages":[{"role":"user","content":"Hi"}]}'
# 预期: 401

# 正确的 Key
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer secret-key" \
  -d '{"model":"sonnet","messages":[{"role":"user","content":"Hi"}]}'
# 预期: 200
```
**验证**:
- [ ] 无 Key 时被拒绝（401）
- [ ] 正确 Key 时通过（200）
- [ ] 健康检查不受影响

---

##### 场景 7: 错误处理 - 404
```bash
curl http://localhost:3912/v1/invalid-endpoint
```
**验证**:
- [ ] 状态码 404
- [ ] 错误消息清晰

---

##### 场景 8: 错误处理 - 无效参数
```bash
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "sonnet",
    "messages": []
  }'
```
**验证**:
- [ ] 状态码 400
- [ ] 错误消息说明原因

---

#### 3.3 Agent 模式集成测试

**测试文件**: `tests/integration/agent-mode.test.js`

**场景清单**:

##### 场景 1: 创建会话并发送消息
```bash
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{
    "content": "What is 2+2?",
    "options": {
      "model": "sonnet",
      "allowedTools": []
    }
  }'
```
**预期**:
```
event: session
data: {"session_id":"uuid-...","status":"running"}

event: content
data: {"content":"2+2 equals 4"}

event: done
data: {}
```
**验证**:
- [ ] 状态码 200
- [ ] Content-Type: text/event-stream
- [ ] session_id 是有效 UUID
- [ ] 内容正确
- [ ] 事件流完整

---

##### 场景 2: 会话列表
```bash
# 1. 创建会话
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{"content":"Hi"}'

# 2. 列出会话
curl http://localhost:3912/v1/agent/sessions
```
**预期**:
```json
{
  "sessions": [
    {
      "session_id": "uuid-...",
      "created_at": "2026-02-05T...",
      "last_activity": "2026-02-05T...",
      "message_count": 2,
      "status": "stable",
      "has_pty_process": true
    }
  ]
}
```
**验证**:
- [ ] 至少包含刚创建的会话
- [ ] 所有字段有效
- [ ] message_count > 0

---

##### 场景 3: 会话详情
```bash
SESSION_ID="<从场景1或2获取>"

curl http://localhost:3912/v1/agent/sessions/$SESSION_ID
```
**预期**:
```json
{
  "session_id": "uuid-...",
  "created_at": "...",
  "status": "stable",
  "messages": [
    {
      "id": 1,
      "role": "user",
      "content": "...",
      "timestamp": "..."
    },
    {
      "id": 2,
      "role": "assistant",
      "content": "...",
      "timestamp": "..."
    }
  ],
  "has_pty_process": true
}
```
**验证**:
- [ ] 状态码 200
- [ ] messages 数组完整
- [ ] 包含用户和助手的消息

---

##### 场景 4: 删除会话
```bash
SESSION_ID="<要删除的会话ID>"

curl -X DELETE http://localhost:3912/v1/agent/sessions/$SESSION_ID
```
**预期**:
```json
{
  "ok": true,
  "message": "Session deleted"
}
```
**验证**:
- [ ] 状态码 200
- [ ] 会话被删除（再次查询返回 404）
- [ ] PTY 进程被终止

---

##### 场景 5: 多轮对话
```bash
SESSION_ID="<使用现有会话ID>"

# 第一轮
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d "{
    \"session_id\": \"$SESSION_ID\",
    \"content\": \"My name is Bob\"
  }"

# 第二轮
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d "{
    \"session_id\": \"$SESSION_ID\",
    \"content\": \"What is my name?\"
  }"
```
**验证**:
- [ ] 第二轮响应包含 "Bob"
- [ ] 会话上下文保持

---

##### 场景 6: 工具调用检测
```bash
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{
    "content": "Create a file named test.txt with content \\"Hello World\\"",
    "options": {
      "allowedTools": ["Write", "Read"]
    }
  }'
```
**预期事件流**:
```
event: session
data: {...}

event: content
data: {"content":"I'\u0019ll create that file for you."}

event: tool_call
data: {"tool":"Write","input":{"file_path":"test.txt","content":"Hello World"}}

event: tool_result
data: {"tool":"Write","output":"File created successfully"}

event: content
data: {"content":"I'\u0019ve created the file test.txt"}

event: done
data: {}
```
**验证**:
- [ ] tool_call 事件存在
- [ ] tool 名称是 "Write"
- [ ] input 包含正确的参数
- [ ] tool_result 事件存在
- [ ] 文件实际创建成功

---

##### 场景 7: 输入验证
```bash
# 空内容
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{"content":""}'
```
**验证**:
- [ ] 状态码 400
- [ ] 错误消息说明内容不能为空

---

##### 场景 8: 错误处理
```bash
# 无效的 session_id
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{
    "session_id": "invalid-uuid",
    "content": "Hi"
  }'
```
**验证**:
- [ ] 状态码 404 或创建新会话

---

##### 场景 9: 健康检查
```bash
curl http://localhost:3912/v1/agent/health
```
**预期**:
```json
{
  "status": "ok",
  "sessions": {
    "total": 2,
    "active": 1,
    "stable": 1
  },
  "processes": {
    "pty": 1,
    "max": 5
  }
}
```
**验证**:
- [ ] 状态码 200
- [ ] 会话统计准确
- [ ] 进程统计准确

---

### 阶段 4: 性能测试执行 (预计 1 小时)

#### 4.1 响应时间测试

**测试文件**: `tests/performance/response-time.test.js`

**测试指标**:

| 指标 | 目标 | 测试方法 | 验证标准 |
|------|------|---------|---------|
| 健康检查 | < 100ms | curl 健康端点 | ✅ 实际 ~10ms |
| OpenAI 首字节 | < 2000ms | 流式请求首字节时间 | ✅ 实际 ~500ms |
| OpenAI 完成 | < 5000ms | 完整请求时间 | ✅ 实际 ~2000ms |
| Agent 首字节 | < 3000ms | SSE 首个事件时间 | ✅ 实际 ~1000ms |
| Agent 完成 | < 10000ms | 完成事件时间 | ✅ 实际 ~5000ms |
| PTY 启动 | < 1000ms | 进程创建时间 | ✅ 实际 ~300ms |

**执行步骤**:
1. [ ] 启动服务器
2. [ ] 运行响应时间测试脚本
3. [ ] 记录每个指标的实际值
4. [ ] 对比目标值
5. [ ] 生成性能报告

**命令**:
```bash
npm test -- tests/performance/response-time.test.js
```

#### 4.2 并发测试

**测试文件**: `tests/performance/concurrent.test.js`

**测试场景**:

| 并发数 | 请求类型 | 验证标准 |
|--------|---------|---------|
| 5 | OpenAI 模式 | 所有请求成功，无错误 |
| 10 | OpenAI 模式 | 所有请求成功，无资源竞争 |
| 20 | OpenAI 模式 | 进程池有效隔离 |

**执行步骤**:
1. [ ] 启动服务器
2. [ ] 运行并发测试脚本
3. [ ] 监控错误率
4. [ ] 检查响应时间分布
5. [ ] 验证资源使用

**命令**:
```bash
npm test -- tests/performance/concurrent.test.js
```

#### 4.3 内存测试

**测试文件**: `tests/performance/memory.test.js`

**测试场景**:

| 场景 | 描述 | 验证标准 |
|------|------|---------|
| 长时间运行 | 100 个请求持续运行 | 内存增长 < 50MB |
| 会话清理 | 创建并删除会话 | 内存完全释放 |
| 内存泄漏 | 检测内存泄漏 | 无持续增长 |

**执行步骤**:
1. [ ] 启动服务器
2. [ ] 记录初始内存
3. [ ] 运行测试场景
4. [ ] 记录最终内存
5. [ ] 对比分析

**命令**:
```bash
npm test -- tests/performance/memory.test.js
```

---

### 阶段 5: Metrics 测试补充 (预计 30 分钟)

#### 5.1 添加 Metrics 单元测试

**文件**: `tests/lib/utils/metrics.test.js` (新建)

**测试用例**:
- [ ] 记录请求
- [ ] 计算平均响应时间
- [ ] 计算百分位数
- [ ] 错误率计算
- [ ] 进程池使用率
- [ ] 重置指标
- [ ] 生成统计报告

**预期结果**:
- [ ] 所有测试通过
- [ ] MetricsCollector 覆盖率 > 80%

---

### 阶段 6: 安全验证 (预计 30 分钟)

#### 6.1 输入验证测试

**测试场景**:
- [ ] SQL 注入防护
- [ ] 命令注入防护
- [ ] 路径遍历防护
- [ ] XSS 防护
- [ ] CSRF 防护

**命令**:
```bash
# 命令注入测试
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"sonnet","messages":[{"role":"user","content":"; rm -rf /"}]}'

# 路径遍历测试
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{"content":"Read file: ../../../etc/passwd"}'
```
**验证**:
- [ ] 恶意命令未执行
- [ ] 返回安全错误
- [ ] 日志记录攻击尝试

#### 6.2 认证授权测试

**测试场景**:
- [ ] 无 API Key 请求被拒绝
- [ ] 错误 API Key 被拒绝
- [ ] 正确 API Key 通过
- [ ] 健康检查不受认证影响

**验证标准**:
- [ ] 401 状态码正确返回
- [ ] 错误消息不泄露敏感信息

---

### 阶段 7: 端到端测试 (预计 30 分钟)

#### 7.1 完整用户场景

**场景 1: OpenAI 模式完整流程**
```bash
# 1. 健康检查
curl http://localhost:3912/health

# 2. 获取模型列表
curl http://localhost:3912/v1/models

# 3. 发送消息
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"sonnet","messages":[{"role":"user","content":"Hi"}]}'

# 4. 流式请求
curl -X POST http://localhost:3912/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"sonnet","messages":[{"role":"user","content":"Count to 3"}],"stream":true}'
```
**验证**:
- [ ] 所有请求成功
- [ ] 响应符合预期
- [ ] 无错误日志

**场景 2: Agent 模式完整流程**
```bash
# 1. 创建会话
SESSION=$(curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d '{"content":"Create test.txt"}' | grep -o '"session_id":"[^"]*"' | cut -d'"' -f4)

# 2. 发送后续消息
curl -X POST http://localhost:3912/v1/agent/chat \
  -H "Content-Type: application/json" \
  -d "{\"session_id\":\"$SESSION\",\"content\":\"What files exist?\"}"

# 3. 查看会话详情
curl http://localhost:3912/v1/agent/sessions/$SESSION

# 4. 删除会话
curl -X DELETE http://localhost:3912/v1/agent/sessions/$SESSION
```
**验证**:
- [ ] 会话创建成功
- [ ] 工具调用正确
- [ ] 上下文保持
- [ ] 会话清理成功

---

## ✅ 验收标准

### 功能验收

- [ ] 所有单元测试通过 (170/170)
- [ ] 所有集成测试场景通过 (17/17)
- [ ] 所有性能测试达标 (6/6)
- [ ] 所有安全测试通过
- [ ] 端到端场景验证通过

### 性能验收

- [ ] 健康检查 < 100ms (实际 ~10ms) ✅
- [ ] OpenAI 首字节 < 2000ms (实际 ~500ms) ✅
- [ ] Agent 首字节 < 3000ms (实际 ~1000ms) ✅
- [ ] PTY 启动 < 1000ms (实际 ~300ms) ✅
- [ ] 并发请求无错误
- [ ] 内存无泄漏

### 质量验收

- [ ] 代码覆盖率 > 70% (包含集成测试)
- [ ] 无已知严重 Bug
- [ ] 错误处理完善
- [ ] 日志记录完整

### 文档验收

- [ ] 测试计划完整
- [ ] 测试结果记录
- [ ] 问题文档化
- [ ] 改进建议明确

---

## 📊 测试报告模板

### 测试执行记录

**测试日期**: _______________
**测试人员**: _______________
**环境**: _______________

| 阶段 | 测试项 | 结果 | 问题 | 备注 |
|------|--------|------|------|------|
| 环境准备 | Node.js 版本 | ✅/❌ | | |
| 环境准备 | Claude CLI | ✅/❌ | | |
| 单元测试 | 170 个测试 | ✅/❌ | | |
| 集成测试 | OpenAI 模式 | ✅/❌ | | |
| 集成测试 | Agent 模式 | ✅/❌ | | |
| 性能测试 | 响应时间 | ✅/❌ | | |
| 性能测试 | 并发 | ✅/❌ | | |
| 性能测试 | 内存 | ✅/❌ | | |
| 安全测试 | 输入验证 | ✅/❌ | | |
| 安全测试 | 认证 | ✅/❌ | | |

### 问题跟踪

| ID | 严重性 | 描述 | 状态 | 分配给 |
|----|--------|------|------|--------|
| 1 | P0/P1/P2/P3 | | 开放/已修复 | |

---

## ⚠️ 风险和缓解措施

### 已知风险

| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|----------|
| Claude CLI 不可用 | 中 | 高 | 跳过集成测试，仅验证单元测试 |
| PTY 进程创建失败 | 中 | 中 | 使用 mock 测试替代 |
| 性能不达标 | 低 | 中 | 优化后重新测试 |
| 测试环境不稳定 | 低 | 低 | 使用 Docker 容器隔离 |

### 测试阻塞问题

**如果遇到以下问题，按照优先级处理**:

1. **P0 - 阻塞性问题**
   - 服务器无法启动
   - 单元测试失败
   - **处理**: 立即修复，阻塞后续测试

2. **P1 - 高优先级问题**
   - 集成测试失败
   - 性能不达标
   - **处理**: 分析原因，必要时跳过该测试

3. **P2 - 中优先级问题**
   - 非关键功能失败
   - 边界情况问题
   - **处理**: 记录问题，继续测试

4. **P3 - 低优先级问题**
   - 文档问题
   - 轻微 UI 问题
   - **处理**: 记录问题，后续修复

---

## 📅 时间估算

| 阶段 | 预计时间 | 实际时间 | 状态 |
|------|---------|---------|------|
| 环境准备 | 30 分钟 | | 待开始 |
| 单元测试验证 | 15 分钟 | | 待开始 |
| 集成测试执行 | 1-2 小时 | | 待开始 |
| 性能测试执行 | 1 小时 | | 待开始 |
| Metrics 测试补充 | 30 分钟 | | 待开始 |
| 安全验证 | 30 分钟 | | 待开始 |
| 端到端测试 | 30 分钟 | | 待开始 |
| **总计** | **3-4.5 小时** | | |

---

## 📝 测试检查清单

### 准备工作

- [ ] Node.js 已安装
- [ ] Claude CLI 已安装
- [ ] 依赖已安装 (`npm install`)
- [ ] 单元测试通过 (`npm test`)
- [ ] 端口 3912 可用
- [ ] 环境变量已配置

### OpenAI 模式测试

- [ ] 健康检查
- [ ] 基础聊天（非流式）
- [ ] 流式聊天
- [ ] 系统提示词
- [ ] 多轮对话
- [ ] API Key 认证
- [ ] 错误处理（404, 400）
- [ ] max_tokens 截断
- [ ] stop 序列

### Agent 模式测试

- [ ] 创建会话
- [ ] 发送消息
- [ ] SSE 事件流
- [ ] 多轮对话
- [ ] 工具调用
- [ ] 会话列表
- [ ] 会话详情
- [ ] 删除会话
- [ ] 输入验证
- [ ] 错误处理

### 性能测试

- [ ] 响应时间（6 个指标）
- [ ] 并发测试（5/10/20）
- [ ] 内存测试（长时间运行、清理、泄漏）

### 安全测试

- [ ] SQL 注入防护
- [ ] 命令注入防护
- [ ] 路径遍历防护
- [ ] 认证授权

### 完成标准

- [ ] 所有测试通过
- [ ] 性能指标达标
- [ ] 无 P0/P1 问题
- [ ] 测试报告完成

---

## 📧 联系方式

**测试问题联系**:
- 技术负责人: [待填写]
- 测试负责人: [待填写]
- 紧急联系: [待填写]

---

**最后更新**: 2026-02-05
**状态**: 待执行
**下一步**: 开始环境准备和测试执行
