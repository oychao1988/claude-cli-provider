...
class ProcessManager {
  ...
  createCLIProcess(options = {}) {
    ...
    // Get current working directory and environment before declaring childProcess
    const cwd = process.cwd() || '/home/nodejs/app';
    const env = process.env;

    // Spawn the process
    let childProcess;
    try {
      logger.debug('Attempting to spawn Claude CLI', {
        claudeBin: this.claudeBin,
        args: args.join(' '),
        cwd,
        envKeys: Object.keys(env).filter(k => k.includes('ANTHROPIC') || k.includes('HOME') || k.includes('NODE'))
      });
      childProcess = spawn(this.claudeBin, args, {
        env,
        cwd
      });
      logger.debug('Claude CLI spawned successfully', {
    ...
    this.cliProcesses.set(processId, childProcess);
  ...
  /**
   * Create a PTY mode process for Agent adapter
  ...
  async createPTYProcess(options = {}) {
    // Check process pool limit
    if (this.ptyProcesses.size >= this.maxProcesses) {
      throw new ProcessError('Maximum PTY process limit reached', {
        limit: this.maxProcesses,
        current: this.ptyProcesses.size
      });
    }
    ...
    let ptyProcess;
    try {
      ptyProcess = pty.spawn(this.claudeBin, args, {
        name: 'xterm-color',
        cols: 80,
        rows: 24,
      ...
      throw new ProcessError('Failed to spawn PTY process', {
        error: error.message,
        claudeBin: this.claudeBin
      });
    }
    ...
    ptyProcess.on('error', (error) => {
      logger.error('PTY process error', {
        processId,
        error: error.message
      });
      this.ptyProcesses.delete(processId);
    ...
    logger.info('PTY process created', {
      ...
      pid: ptyProcess.pid,
      model,
      activeProcesses: this.ptyProcesses.size
    });
